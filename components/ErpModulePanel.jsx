...
export default function ErpModulePanel({ user, companyContext, isOnline, summary, onOpen }) {
  ...
  return (
    <div className="native-module-panel min-w-0 space-y-4 pb-24 lg:pb-8">
      <section className="native-module-hero relative overflow-hidden rounded-[2rem] border border-emerald-950/10 bg-[linear-gradient(125deg,#09150f_0%,#10291d_54%,#1f4429_100%)] p-6 text-white shadow-[0_28px_75px_-42px_rgba(5,46,22,0.82)] sm:p-8">
        ...
        <div className="relative grid gap-7 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          <div>
            ...
            <p className="native-module-intro mt-3 max-w-xl text-sm font-semibold leading-6 text-emerald-50/70 sm:text-base">Selecciona una operacion.</p>
          </div>
          ...
          {availableModules.map((module) => {
            ...
            return (
              <button
                ...
                className={`native-module-card group relative overflow-hidden rounded-[1.45rem] border bg-white p-4 text-left shadow-[0_18px_45px_-34px_rgba(15,23,42,0.55)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_26px_55px_-32px_rgba(15,23,42,0.45)] lg:min-h-56 lg:p-5 ${tone.card}`}
              >
                <span className={`absolute -right-10 -top-10 h-36 w-36 rounded-full blur-2xl transition group-hover:scale-125 ${tone.glow}`} />
                <span className={`native-module-card-icon relative grid h-12 w-12 place-items-center rounded-2xl ${tone.icon}`}>
                  <span className="h-6 w-6"><ModuleIcon name={module.icon} /></span>
                </span>
                <span className="native-module-card-title relative mt-7 block text-xl font-black tracking-[-0.035em] text-slate-950">{module.title}</span>
                <span className="native-module-card-copy relative mt-2 block max-w-sm text-sm font-semibold leading-5 text-slate-500">{module.description}</span>
                <span className={`native-module-card-action relative mt-6 flex items-center justify-between text-xs font-black uppercase tracking-[0.1em] ${tone.action}`}>
                  {module.action}
      ...
      {companyContext?.internalTransfers ? <section>
        <div className="native-activity-card rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm">
