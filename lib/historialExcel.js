...
function buildDetailedRows(pedidos, role) {
  ...
  return pedidos.flatMap((pedido) =>
    getPedidoItems(pedido).map((item) => ({
      ...
      "Costo unitario": formatCostValue(getItemUnitCost(item, pedido)),
      "Importe linea": formatCostValue(getItemLineCost(item, pedido)),
...
function buildConsolidatedExportRows(pedidos) {
  return buildConsolidatedRows(pedidos).map((row) => ({
    ...
    "Costo promedio": formatCostValue(row.averageCost),
    "Costo total": formatCostValue(row.totalCost),
    Pedidos: row.pedidos,
...
async function createWorkbookFromRows({
  ...
  const metadataRows = [
    ...
    ["Costo total", typeof totalCost === "number" ? totalCost.toFixed(2) : "Sin costo"],
    [],
  ];
...
async function downloadConsolidatedHistoryExcel({ title, pedidos, fechaInicio, fechaFin, user, sheetName }) {
  ...
  const hasCosts = rows.some((row) => typeof row["Costo total"] === "number");
  const totalCost = hasCosts
    ? rows.reduce((sum, row) => sum + (typeof row["Costo total"] === "number" ? row["Costo total"] : 0), 0)
    : null;
