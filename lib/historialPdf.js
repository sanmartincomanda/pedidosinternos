...
import { printPdfDocument } from "@/lib/nativePdfPrinter";
...
function buildDoc(title, orientation = "portrait") {
...
function buildReportSummaryLines({ user, fechaInicio, fechaFin, cantidadPedidos, totalReal, totalCost }) {
  return [
    { label: "Sucursal", value: getBranchDisplayName(user), width: 170 },
    { label: "Desde", value: formatDateLabel(fechaInicio), width: 110 },
    { label: "Hasta", value: formatDateLabel(fechaFin), width: 110 },
    { label: "Pedidos", value: cantidadPedidos, width: 90 },
    { label: "Peso", value: `${totalReal.toFixed(2)} lb`, width: 110 },
    { label: "Costo", value: formatCurrencyDisplay(totalCost), width: 130 },
  ];
}

function buildDetailRows(pedidos, role) {
  return pedidos.flatMap((pedido) =>
    getPedidoItems(pedido).map((item) => [
      ...
      formatCostValue(getItemUnitCost(item, pedido)),
      formatCostValue(getItemLineCost(item, pedido)),
...
function buildConsolidatedTableRows(pedidos) {
  return buildConsolidatedRows(pedidos).map((row) => [
    ...
    formatCostValue(row.averageCost),
    formatCostValue(row.totalCost),
    String(row.pedidos),
    row.sucursales || "-",
  ]);
}
...
function downloadDetailedHistoryPdf({ title, pedidos, fechaInicio, fechaFin, user, role }) {
  const doc = buildDoc(title, "landscape");
  const totalReal = pedidos.reduce((sum, pedido) => sum + (pedido.totalReal || 0), 0);
  const hasCosts = pedidos.some((pedido) => pedido.hasCosts);
  const totalCost = hasCosts ? pedidos.reduce((sum, pedido) => sum + (pedido.totalCost || 0), 0) : null;
  addMetaRow(
    doc,
  ...
  autoTable(doc, {
    startY: 104,
    head: [["Pedido", "Fecha", role === "recibidos" ? "Enviado por" : "Recibe", "Clave", "Producto", "Solicitado", "Real", "Costo unit.", "Importe", "Estado"]],
    body: buildDetailRows(pedidos, role),
    styles: { fontSize: 7.4, cellPadding: 4, textColor: [15, 23, 42] },
...
function downloadConsolidatedHistoryPdf({ title, pedidos, fechaInicio, fechaFin, user }) {
  ...
  autoTable(doc, {
    startY: 104,
    head: [["Clave", "Producto", "Unidad", "Cantidad total", "Costo prom.", "Costo total", "Pedidos", "Sucursales"]],
...
function buildTransferRequisitionDoc(pedido) {
  ...
  const totalReal = items.reduce((sum, item) => {
    const realText = getRealText(item, pedido);
    const numeric = Number.parseFloat(`${realText}`.replace(/[^\d.-]/g, ""));
    return sum + (Number.isFinite(numeric) ? numeric : 0);
  }, 0);
  const totalCost = typeof pedido.totalCost === "number"
    ? pedido.totalCost
    : items.reduce((sum, item) => sum + (getItemLineCost(item, pedido) || 0), 0);
  ...
  addMetaRow(
    doc,
    [
      { label: "Peso real", value: `${totalReal.toFixed(2)} lb`, width: 170 },
      { label: "Costo total", value: formatCurrencyDisplay(totalCost), width: 220 },
    ],
    124,
  );
  ...
  autoTable(doc, {
    startY: 148,
    head: [["Clave", "Producto", "Solicitado", "Peso real", "Costo unit.", "Importe", "Nota"]],
    body: items.map((item) => [
      item?.clave || "",
      item?.producto || "",
      getRequestedText(pedido, item),
  ...
  doc.setFontSize(11);
  doc.setTextColor(15, 23, 42);
  doc.text(`Total costo del traspaso: ${formatCurrencyDisplay(totalCost)}`, 40, finalY);

  if (pedido.notaGeneral) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    doc.setTextColor(15, 23, 42);
    doc.text("Nota general:", 40, finalY + 22);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(71, 85, 105);
    doc.text(doc.splitTextToSize(pedido.notaGeneral, 520), 110, finalY + 22);
  }

  addSignatureFooter(doc, Math.max(doc.lastAutoTable.finalY + 118, 680), ["Entrega", "Recibe"]);
  return doc;
}

function getTransferRequisitionFileName(pedido) {
  return `Requisa_${formatOrderNumber(pedido)}.pdf`.replace(/[\\/:*?"<>|]/g, "-");
}

function downloadTransferRequisitionPdf({ pedido }) {
  saveDoc(buildTransferRequisitionDoc(pedido), getTransferRequisitionFileName(pedido));
}
    ...
    .map(
      (item) => `
        <div class="item">
          ...
          <div class="item-meta">Costo unit.: ${formatCurrencyDisplay(getItemUnitCost(item, pedido))}</div>
          <div class="item-meta">Importe: ${formatCurrencyDisplay(getItemLineCost(item, pedido))}</div>
      ...
      <body>
        <div class="ticket">
          ...
          <div class="meta">
            ...
            <div class="box"><div class="label">Peso total</div><div class="value">${totalReal.toFixed(2)} lb</div></div>
            <div class="box"><div class="label">Costo total</div><div class="value">${formatCurrencyDisplay(totalCost)}</div></div>
          </div>
...
async function printTransferRequisition(pedido) {
  if (typeof window === "undefined") return { ok: false };

  const doc = buildTransferRequisitionDoc(pedido);
  const dataUri = doc.output("datauristring");
  const base64 = dataUri.slice(dataUri.indexOf(",") + 1);
  return printPdfDocument({
    base64,
    fileName: getTransferRequisitionFileName(pedido),
  });
}
