...
function resolveInventoryLogin(value = "") {
  const login = normalizeText(value).toLowerCase();
  return INVENTORY_LOGIN_ALIASES[login] || login;
}
...
function buildInventoryDocuments(payload, authUser) {
  ...
  const items = inputItems.map((item, index) => {
    ...
    return {
      ...
      totalLb: isWeight ? cantidad : undefined,
      pesos: isWeight ? pesos : undefined,
    };
  });
...
export async function loginInventoryUser(login, password) {
  await setPersistence(inventoryAuth, browserLocalPersistence);
  const credential = await signInWithEmailAndPassword(
    inventoryAuth,
    resolveInventoryLogin(login),
    password,
  );
