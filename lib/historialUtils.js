import { formatOrderNumber, getPedidoCreationTimestamp, getPedidoItems } from "@/lib/orderUtils";
...
function getOrderTotals(pedido) {
  ...
  if (!totals.hasCosts) {
    const topLevelCost = pickFirstNumericValue([
      pedido?.totalCostoSicar,
      pedido?.montoMonetarioSicar,
      pedido?.totalCost,
      pedido?.amount,
      pedido?.monto,
      pedido?.contabilidadSync?.amount,
    ]);

    if (topLevelCost !== null) {
      return {
        ...totals,
        totalCost: Number(topLevelCost.toFixed(2)),
        hasCosts: true,
      };
    }
  }
...
function matchesSearch(pedido, searchTerm) {
  ...
  const haystack = [
    ...
    ...getPedidoItems(pedido).flatMap((item) => [item?.clave, item?.producto, item?.nota]),
  ]
