...
test("Masaya web prioriza el Funnel publico y conserva la ruta privada", () => {
  const endpoints = buildSicarApiCandidates({
    companyKey: "masaya",
    savedEndpoint: "https://servidor-masaya.tail95b6f5.ts.net",
    lastWorkingEndpoint: "https://servidor-masaya.tail95b6f5.ts.net",
    pageHostname: "traspasos.sanmartinsr.com",
    pageProtocol: "https:",
  });

  assert.deepEqual(endpoints, [
    "https://servidor-masaya.tail95b6f5.ts.net:8443/masaya-api",
    "https://servidor-masaya.tail95b6f5.ts.net",
  ]);
});

test("APK Masaya puede priorizar la ultima ruta privada conocida", () => {
  const endpoints = buildSicarApiCandidates({
    companyKey: "masaya",
    lastWorkingEndpoint: "https://servidor-masaya.tail95b6f5.ts.net",
    nativePlatform: true,
