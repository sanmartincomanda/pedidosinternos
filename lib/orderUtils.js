...
export function getHighestBranchSequence(pedidos = [], sucursal = "") {
  ...
  return pedidos.reduce((maximo, pedido) => {
    ...
    const sucursalPropietaria = pedido.sucursalCreadora || pedido.sucursalOrigen;
    if (sucursalPropietaria !== sucursal) {
      return maximo;
    }
...
export function normalizePedidoItems(items) {
  ...
  const looksIndexedCollection = keys.every((key) => /^\d+$/.test(key));
  if (looksIndexedCollection) {
    return keys
...
export function getPedidoItems(pedido = null) {
  return normalizePedidoItems(pedido?.items);
}

export function normalizePedidoForUi(pedido = {}, firebaseId = "") {
  return {
    ...pedido,
    firebaseId: firebaseId || pedido?.firebaseId || "",
