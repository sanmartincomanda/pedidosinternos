...
import { demoSicarApi } from "@/lib/designPreview";
...
export function checkSicarPurchaseApi() {
  if (activeCompany?.demo) return demoSicarApi.health();
...
export function getSicarOfflineCatalog() {
  if (activeCompany?.demo) return demoSicarApi.offlineCatalog();
  return requestSicarApi("/catalogos/offline");
}

export function getSicarPurchaseHistory(limit = 150) {
  if (activeCompany?.demo) return demoSicarApi.history();
...
export function previewSicarPurchase(payload) {
  if (activeCompany?.demo) return demoSicarApi.preview(payload);
...
export function receiveSicarPurchase(payload) {
  if (activeCompany?.demo) return demoSicarApi.receive(payload);
